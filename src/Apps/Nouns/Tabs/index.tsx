...
// Utility function to parse contract names and separate main name from bracketed description
const parseContractName = (name: string) => {
  const bracketMatch = name.match(/^([^[]+)(\s*\[.*\])?$/);
  if (bracketMatch) {
    const mainName = bracketMatch[1].trim();
    const description = bracketMatch[2] ? ` ${bracketMatch[2].trim()}` : '';
    return { mainName, description };
  }
  return { mainName: name, description: '' };
};
              ...
              <div key={index} className={styles.contractItem}>
                <div className={styles.contractHeader}>
                  <span className={styles.contractName}>
                    <span className={styles.contractNameBold}>{parseContractName(contract.name).mainName}</span>
                    {parseContractName(contract.name).description}:
                  </span>
                  <a 
          ...
          <div className={styles.contractsList}>
            {metagovOrganizations.map((org, index) => (
              <div key={index} className={styles.contractItem}>
                <div className={styles.contractHeader}>
                  <span className={styles.contractName}>
                    <span className={styles.contractNameBold}>{parseContractName(org.name).mainName}</span>
                    {parseContractName(org.name).description}:
                  </span>
                  <div className={styles.chainLinks}>
          ...
          <div className={styles.contractsList}>
            {nounsTokens.map((token, index) => (
              <div key={index} className={styles.contractItem}>
                <div className={styles.contractHeader}>
                  <span className={styles.contractName}>
                    <span className={styles.contractNameBold}>{parseContractName(token.name).mainName}</span>
                    {parseContractName(token.name).description}:
                  </span>
                  <div className={styles.chainLinks}>
